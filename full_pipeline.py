...
def main():
    ...
    n_runs = min(20, len(stable_weights))   # e.g. test first 50 admissible weights
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig(f'{results_dir}/Final performance distribution_{final_acc_stable.mean().round(2)}_{final_acc_random.mean().round(2)}.png', dpi=150, bbox_inches='tight')
